//! The figure `cargo run --example phylo_dnds` writes, as a function.
//!
...
use karyon::{Drawing, Figure, Panels, Region, Theme, TraitColumn, TreeTrack};

/// `example-phylo-dnds.svg`: branch-wise dN/dS across four projections.
///
/// `theme` replaces the light theme on the sheet and on every panel. A sheet
/// of trees is as wide as its panels and has no window along a genome, so
/// `width` and `region` are ignored.
pub fn example_phylo_dnds(
    theme: &Theme,
    _width: Option<f64>,
