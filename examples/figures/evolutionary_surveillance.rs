//! The figure `cargo run --example evolutionary_surveillance` writes, as a
//! function.
//!
...
use karyon::{
    ...
    BranchRateMixture, Drawing, Feature, FeatureTrack, Figure, HomoplasyLayer, Panels,
    PhylodynamicPoint, PhylodynamicScale, PhylodynamicTrack, Region, SelectionEvidence,
    SelectionSite, SelectionTrack, SurveillanceObservation, SurveillanceStyle, SurveillanceTrack,
    TangleTieStyle, TanglegramTrack, Theme, TraitColumn, TreeTrack, Variant, VariantTrack,
};

/// `example-evolutionary-surveillance.svg`: eight panels of evolutionary
/// genomics and surveillance on one sheet.
///
/// `theme` replaces the light theme on the sheet and on every panel. A sheet
/// is as wide as its panels and has no window of its own, so `width` and
/// `region` are ignored.
pub fn example_evolutionary_surveillance(
    theme: &Theme,
    _width: Option<f64>,
    _region: Option<&Region>,
) -> Box<dyn Drawing> {
    let orthogonal = geometry_panel(theme, BranchGeometry::Orthogonal, "Orthogonal phylogram");
    let diagonal = geometry_panel(theme, BranchGeometry::Diagonal, "Diagonal phylogram");
    let curved = ancestral_panel(theme);
    let circular = circular_evolution_panel(theme);
    let unrooted = unrooted_evolution_panel(theme);
    let comparison = comparison_panel(theme);
    let genomic = genomic_selection_panel(theme);
    let surveillance = surveillance_panel(theme);
...
fn geometry_panel(theme: &Theme, geometry: BranchGeometry, title: &str) -> Figure {
...
fn ancestral_panel(theme: &Theme) -> Figure {
    Figure::new(tree_region())
...
fn circular_evolution_panel(theme: &Theme) -> Figure {
...
fn unrooted_evolution_panel(theme: &Theme) -> Figure {
    Figure::new(tree_region())
...
fn comparison_panel(theme: &Theme) -> Figure {
...
fn genomic_selection_panel(theme: &Theme) -> Figure {
...
fn surveillance_panel(theme: &Theme) -> Figure {
