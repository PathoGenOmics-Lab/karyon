//! The figure `cargo run --example gallery` writes, as a function.
//!
...
/// `gallery.svg`: most kinds of plot this crate draws, on one sheet.
///
/// `theme` replaces the light theme on the sheet and on every panel, the
/// circle included. A sheet is as wide as its panels and has no window of its
...
pub fn gallery(theme: &Theme, _width: Option<f64>, _region: Option<&Region>) -> Box<dyn Drawing> {
    let sheet = Panels::new()
        ...
        .push_captioned(
            &variable_sites(theme),
            "G",
            "Variable sites with the phylogeny that orders them",
        )
        .push_captioned(&phylogeny(theme), "H", "A tree on its own")
        .push_captioned(
            &selection(theme),
            "I",
...
fn association(theme: &Theme) -> Figure {
...
/// E: two sequences.
fn comparison(theme: &Theme) -> Figure {
    let blocks = vec![
...
/// F: an alignment.
fn alignment(theme: &Theme) -> Figure {
    let rows = vec![
...
fn variable_sites(theme: &Theme) -> Figure {
...
/// H: a tree by itself.
fn phylogeny(theme: &Theme) -> Figure {
...
fn selection(theme: &Theme) -> Figure {
...
fn squiggle(theme: &Theme) -> Figure {
...
fn methylation(theme: &Theme) -> Figure {
...
fn genome_wide(theme: &Theme) -> Figure {
...
fn structural(theme: &Theme) -> Figure {
...
fn tanglegram(theme: &Theme) -> Figure {
...
fn bisulfite(theme: &Theme) -> Figure {
...
fn split_reads(theme: &Theme) -> Figure {
...
/// V: one RNA at a time.
fn transcripts(theme: &Theme) -> Figure {
